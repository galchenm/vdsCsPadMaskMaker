...
def converter_to_slab_format(data, num = 0):
    data_shape = data.shape
    if len(data_shape) > 3:
        new_shape = (data_shape[1] * data_shape[2], data_shape[3])
        data = np.reshape(data[num,].ravel(), new_shape)
    else:
...
def geometry_converter_jf(in_geom_file_name):
        ...
        if line.startswith(';dim2 '):
            ...
            if len(s) == 2 and s[1] in ('min_ss', 'max_ss'):
                ...
                if len(s) == 2:
                    ...
                    v = int(s[1]) + (num_panel-1) * 512 # or (num_panel -1)*512+ss
                    # num_panel += 1 
                    out_geom.write('%s = %d\n'%(s[0], v))
            else:
                out_geom.write(line)
...
def read_geometry_file_preamble(fnam):
    ...
    dim = {}
    dim_info = list(filter(re.compile('\%').search, f_lines)) + list(filter(re.compile('= ss').search, f_lines)) + list(filter(re.compile('= fs').search, f_lines))
    for dd in dim_info:
        numdim, k = dd.split('=')
        numdim = numdim if '/' not in numdim else numdim.split('/')[1]
        numdim = int(re.search(r'\d',numdim).group())
        dim[k.strip()] = numdim
    ...
    preamble['data'] = [l for l in f_lines if l.startswith('data') and not(l.startswith(";"))]
    if len(preamble['data']) == 0:
        preamble['data'] = None
    else:
        preamble['data'] = preamble['data'][0].rsplit()[2]
